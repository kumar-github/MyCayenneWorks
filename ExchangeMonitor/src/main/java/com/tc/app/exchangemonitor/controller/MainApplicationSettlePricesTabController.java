...
public class MainApplicationSettlePricesTabController implements IMainApplicationMonitorTabController
{
	...
	private void initializeAnimationsIfNeeded()
	{
		if(this.isAnimationNeeded())
		{
			...
			final RotateTransition r = new RotateTransition(Duration.seconds(2), this.settlePricesTableView);
			r.setFromAngle(0);
			r.setByAngle(360);
			r.play();

			final FadeTransition ft = new FadeTransition(Duration.seconds(2), this.settlePricesTableView);
			ft.setFromValue(1.0);
			ft.setToValue(0.0);
			ft.setCycleCount(2);
